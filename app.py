...
def get_weak_topics(user_id):
    ...
    for topic, stats in user_performance[user_id].items():
...
def update_user_performance(user_id, topic):
    ...
    cursor.execute("""
        SELECT COUNT(*), AVG(score), AVG(accuracy)
        FROM quiz_attempts
    ...
    attempts_count = result[0]
    average_score = result[1] if result[1] is not None else 0
    accuracy = result[2] if result[2] is not None else 0
...
def submit_answer():
    ...
    # Initialize topic
    if topic not in user_performance[user_id]:
        user_performance[user_id][topic] = {"correct": 0, "total": 0}

    # Update stats
    user_performance[user_id][topic]["total"] += 1

    if is_correct:
        user_performance[user_id][topic]["correct"] += 1

    print("UPDATED PERFORMANCE:", user_performance)
